...
from app.services.auth import create_access_token, create_refresh_token, decode_access_token, verify_password
...
async def register(
    ...
    try:
        new_user = await user_service.register_user(user.username, user.email, user.password)
...
async def login(
    ...
    try:
        ...
        access_token = create_access_token(data={"sub": authenticated_user.username, "role": authenticated_user.role.value})
        refresh_token = create_refresh_token(data={"sub": authenticated_user.username, "role": authenticated_user.role.value})
...
async def refresh_token(refresh_token: str):
    try:
        ...
        return RefreshTokenResponseSchema(access_token=new_access_token, token_type="bearer")
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
